<script>
    ...
    onMount(() => {
        ...
        if (!trainings.some((t) => String(t.sport || "").trim() === "Laufen")) {
            const availableSport = ["Radfahren", "Schwimmen", "Laufen"].find(
                (sport) =>
                    trainings.some(
                        (t) => String(t.sport || "").trim() === sport,
                    ),
            );
    ...
    let filteredTrainings = $derived(
        (data.trainings || []).filter((training) => {
            const term = String(searchTerm || "")
                .trim()
                .toLowerCase();
            const sport = String(filterSport || "")
                .trim()
                .toLowerCase();
            const trainingSport = String(training.sport || "")
                .trim()
                .toLowerCase();
    ...
    async function saveEdit(id) {
        ...
        const response = await fetch("?/updateTraining", {
            method: "POST",
            body: fd,
        });
        if (response.ok) {
    ...
    async function confirmDelete(id) {
        const confirmed = confirm(
            "Möchtest du dieses Training wirklich unwiderruflich löschen?",
        );
        if (!confirmed) return;
        const fd = new FormData();
        fd.append("id", id);
        const response = await fetch("?/deleteTraining", {
            method: "POST",
            body: fd,
        });
        if (response.ok) {
                        ...
                        <div class="best-records">
                            {#each bests as record}
                                <div class="best-record">
                                    <span class="best-record-label"
                                        >{record.label}</span
                                    >
                                    {#if record.time === "--:--"}
                                        <span
                                            class="best-record-time placeholder"
                                            >{record.time}</span
                                        >
                                    {:else}
                ...
                <div style="position: relative;">
                        ...
                        {#if openOptionsId === training._id}
                            <div class="options-dropdown">
                                ...
                                <button
                                    ...
                                    onclick={() => confirmDelete(training._id)}
                                    >🗑️ Löschen</button
                                >
                            </div>
                        {/if}
                    ...
                    {#if editingId === training._id}
                        <div class="training-card editing-card">
                            <h3 style="margin:0 0 0.25rem 0;font-weight:700">
                                {training.sport} (Bearbeiten)
                            </h3>
                            <div class="edit-grid">
                                <label class="edit-field">
                                    ...
                                    <input
                                        ...
                                        min="0"
                                        step="0.01"
                                        bind:value={editValues.distance}
                                        class="custom-input"
                                    />
                                </label>
                                <label class="edit-field">
                                ...
                                <label class="edit-field">
                                    <span class="edit-label"
                                        >Schmerzlevel (1-10)</span
                                    >
                                    <select
                                        bind:value={editValues.painLevel}
                                        class="custom-select"
                                    >
                                ...
                                <label class="edit-field">
                                    <span class="edit-label"
                                        >Mental Score (1-5)</span
                                    >
                                    <select
                                        bind:value={editValues.mentalScore}
                                        class="custom-select"
                                    >
                    ...
                    {:else}
                        ...
                        >
                            <div class="training-summary">
                                ...
                                <h3
                                    style="margin:0 0 0.25rem 0;font-weight:700;color:#0f172a;"
                                >
                                    {training.sport}
                                </h3>
                                <p style="margin:0;color:#475569">
                                    {Number(training.distance || 0).toFixed(1)} km
                                    | {training.duration} min
                                </p>
                                <p
                                    style="margin-top:0.25rem;color:#64748b;font-size:0.9rem"
                                >
                                    Kalorien: {training.calories} kcal
                                </p>
                            </div>
                        </a>
                    {/if}
            ...
            <div class="search-panel">
                <div class="filter-tabs">
                    {#each ["Laufen", "Schwimmen", "Radfahren"] as sport}
                        <button
                            type="button"
                            class="filter-tab {filterSport === sport
                                ? 'active'
                                : ''}"
                ...
                <label class="search-input-label">
                    ...
                    <input
                        ...
                        placeholder="z. B. Berlin, Intervall, Wettkampf"
                        class="custom-input"
                    />
                </label>
                        ...
                        {#each filteredTrainings as training}
                            <div style="position: relative;">
                                ...
                                {#if editingId === training._id}
                                    <div class="training-card editing-card">
                                        <h3
                                            style="margin:0 0 0.25rem 0;font-weight:700"
                                        >
                                            {training.sport} (Bearbeiten)
                                        </h3>
                                        <div class="edit-grid">
                                    ...
                                    >
                                        <div class="training-summary">
                                            ...
                                            <p style="margin:0;color:#475569">
                                                {Number(
                                                    training.distance || 0,
                                                ).toFixed(1)} km | {training.duration}
                                                min
                                            </p>
                                            <p
                                                style="margin-top:0.25rem;color:#64748b;font-size:0.9rem"
